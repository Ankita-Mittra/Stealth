...
//            guard rootNode.depth == HDNode.defaultPathPrefix.components(separatedBy: "/").count - 1 else {throw AbstractKeystoreError.encryptionError("Derivation depth mismatch")}
    ...
    public convenience init?(mnemonics: String, password: String = "", mnemonicsPassword: String = "", language: BIP39Language = BIP39Language.english, prefixPath: String = HDNode.defaultPathMetamaskPrefix, aesMode: String = "aes-128-cbc") throws { // password: web3swift
    ...
    public init? (seed: Data, password: String = "", prefixPath: String = HDNode.defaultPathMetamaskPrefix, aesMode: String = "aes-128-cbc") throws { // password: web3swift
    ...
    public func createNewChildAccount(password: String = "") throws { // password: web3swift
    ...
    func createNewAccount(parentNode: HDNode, password: String = "") throws { // password: web3swift
    ...
    public func createNewCustomChildAccount(password: String = "", path: String) throws {guard let decryptedRootNode = try? self.getPrefixNodeData(password) else { // password: web3swift
            throw AbstractKeystoreError.encryptionError("Failed to decrypt a keystore")
        }
        guard let rootNode = HDNode(decryptedRootNode) else {
            throw AbstractKeystoreError.encryptionError("Failed to deserialize a root node")
...
//        return Data(bytes:decryptedPK!)
    ...
    public func serializeRootNodeToString(password: String = "") throws -> String { // password: web3swift
        guard let decryptedRootNode = try? self.getPrefixNodeData(password) else {
            throw AbstractKeystoreError.encryptionError("Failed to decrypt a keystore")
        }
        guard let rootNode = HDNode(decryptedRootNode) else {
