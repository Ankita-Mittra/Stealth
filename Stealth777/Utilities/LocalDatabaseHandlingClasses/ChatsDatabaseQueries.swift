        ...
        for msg in messageList{
            if !isMessageExist(id: msg.msgId ?? ""){
                ...
                let message = Messages(context: context)
                    ...
                    message.msg = Data() //need config wth msg.message
                    message.msgId = msg.msgId
                    message.msgType = Int16(msg.msgType!)
                   // message.qouteType = msg.
                    message.quoteMsg = Data() //need config with msg.quoteMsg
                    message.quoteMsgId = msg.quoteMsgId
                    message.readTime = CommonFxns.getDateFromMilliSeconds(mSeconds: Double(msg.readTime ?? "") ?? 0)
                    ...
                    message.sendTime = CommonFxns.getDateFromMilliSeconds(mSeconds: Double(msg.sendTime ?? "") ?? 0)
